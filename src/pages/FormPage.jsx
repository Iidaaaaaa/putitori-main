    ...
    <div className="h-full max-w-md p-4 mx-auto">
      <div className="p-4 bg-white border rounded-md shadow-lg">
        <div className="mt-4 mb-8 text-center ">
          <img className="inline-block" src="/public/logo.png" alt="logo" />
          <p className="mt-2 text-xs">次の居酒屋どこ？？</p>
        </div>

        <h1 className="mb-4 text-lg font-medium">プランを探す</h1>
          ...
          <>
            <div className="space-y-4">
              ...
              {[...Array(places)].map((_, index) => (
                <PlanForm
                  key={index}
                  text={`${index + 1}件目のジャンル: ${
                    genreNames[index] || "選択してください"
                  }`}
                  Icon={<Genre />}
                  onClick={() =>
                    handleOpenModal(
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center mt-4 bg-black bg-opacity-50">
          <div className="p-6 bg-white rounded-lg">{modalContent}</div>
        </div>
      )}
      <h2 className="mt-8 mb-4 text-xl font-bold">店舗情報一覧</h2>
      {shops.length > 0 ? (
        <ul>
          {shops.map((shop, index) => (
            <li key={index} className="mb-2">
              <strong>{index + 1}件目:</strong> {shop.name}（ジャンル:{" "}
              {shop.genre.name}）
            </li>
          ))}
        </ul>
