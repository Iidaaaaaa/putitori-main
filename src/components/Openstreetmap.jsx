 ...
 * @param {Array} params.waypoints - 経由地の座標リスト（[{ lat, lng }, ...]形式）
 * @param {string} params.transport - 移動手段（"foot" | "driving-car" | "cycling"など）
 * @returns {Promise<Object>} - 取得したルートデータ
 */
export const fetchORSRoute = async ({ start, waypoints, transport }) => {
  ...
  try {
    const response = await axios.post(
      ...
      {
        coordinates: [
          [start.lng, start.lat], // 現在地の座標
          ...waypoints.map((wp) => [wp.lng, wp.lat]), // 各店舗の座標
        ],
      },
      {
        headers: {
    ...
    console.log("ORSレスポンス:", response.data); // ★ 追加
